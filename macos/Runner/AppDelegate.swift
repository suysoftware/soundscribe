...
class AppDelegate: FlutterAppDelegate, NSMenuDelegate {
    ...
    override func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let panelRect = NSRect(x: 0, y: 0, width: 200, height: 100)
        ...
        let buttonRect = NSRect(x: 50, y: 20, width: 100, height: 40)
        let customButton = SelectionBarCustomButton(frame: buttonRect)
        customButton.title = "Click me"
        self.customPanel?.level = .floating
        self.customPanel?.contentView?.addSubview(customButton)
        
        
        
        
        self.mouseEventMonitor = NSEvent.addGlobalMonitorForEvents(matching: [.rightMouseDown, .leftMouseDown, .leftMouseDragged]) { [weak self] event in
            ...
            if event.type == .leftMouseDown {
                ...
                print(NSApplication.shared.accessibilitySelectedTextRange())
                //
                ...
                print(selectedRange)
                if selectedRange?.length ?? 0 > 0, let customPanel = self.customPanel {
